<template>
  <div class="job-info">
    ...
    <el-form label-width="220px">
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="saveWorkInfo">保存更新</el-button>
...
import { getWorkInfo, saveWorkInfo } from '@/api/employ'
export default {
  data() {
    return {
  ...
  methods: {
    async getWorkInfo() {
      try {
        const res = await getWorkInfo(this.userId)
        this.formData = res
      } catch (error) {
    ...
    async saveWorkInfo() {
      try {
        await saveWorkInfo(this.formData)
